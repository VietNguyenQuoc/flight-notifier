...
const getAllFlightsByEmail = async email => {
  return await client.hgetall(`monitoring_flights:${email}`);
}

const getFlightByKey = async (email, key) => {
  return await client.hget(`monitoring_flights:${email}`, key);
}

const removeFlight = async (email, key) => {
  return await client.hdel(`monitoring_flights:${email}`, key);
}

const isFlightExist = async (email, key) => {
  return await client.hexists(`monitoring_flights:${email}`, key)
}

module.exports = {
  fetchFlights,
  getAllFlightsByEmail,
  getFlightByKey,
  isFlightExist,
  removeFlight
}
