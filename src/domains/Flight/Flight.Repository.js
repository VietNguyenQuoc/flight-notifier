const { Flight } = require('../../infra/db/sequelize/models');
const redisClient = require('../../infra/db/in-memory/redis');
...
const fetchFlights = async searchData => {
...
const findOrCreateFlight = async (conditions, defaults) => {
  return await Flight.findCreateFind({
    where: conditions,
    defaults
  })
}
...
const getFlightByExternalId = async externalId => {
  return await Flight.findOne({ where: { externalId } });
}

const updateFlight = async (flightDto, conditions) => {
  return await Flight.update(flightDto, { where: conditions });
}

const getFlightInMemoryValue = async (flightId, key) => {
  return await redisClient.get(`flight:${flightId}:${key}`);
}

const setFlightInMemoryValue = async (flightId, key, value) => {
  return await redisClient.set(`flight:${flightId}:${key}`, value);
}

module.exports = {
  getAllFlights,
  getFlightByExternalId,
  findOrCreateFlight,
  updateFlight,
  fetchFlights,
  getFlightInMemoryValue,
  setFlightInMemoryValue,
}